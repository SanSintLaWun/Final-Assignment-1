...
class A3Encryption():
    ...
    def start_encryption(self, text, key):
        ...
        totalKey = 0
        for i in key:
            totalKey += ord(i) # ord = show character to decimal

        key = int(bin(totalKey)[2:]) # bin = binary   [2:] = (0b)101011101 = remove 0b

        for i in text:
            # print(int(bin(ord(i))[2:]) , int(bin(totalKey)[2:]))
            # print("check",type(ord(totalKey)))
            encrypted_ord = ord(i) ^ totalKey    # ^ = XOR
            #                text  (XOR)  key
            #           bin(N=78)  (XOR)  bin
            # result of encrypted_ord = int
...
if __name__ == "__main__":
    a3 = A3Encryption()
    da3 = A3Decryption()
    encrypted: str = a3.start_encryption("NationalCyberCity", "apple")
                                    #         text              key
